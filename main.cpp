...
char *strnstr(const char *big, const char *little, size_t len){
	...
	blen=strlen(big);
	i=0;
	if(blen<llen||len<llen) return (0);
	while(i+llen<=len){
		if (big[i]==*little&&!strncmp(big+i, little, llen)) return((char*)big+i);
		i++;
	}
	return (0);
}
...
int main(int argc, char* argv[]) {
	...
	TcpHdr* tcpHdr=(TcpHdr*) ((size_t)ipHdr+(ipHdr->header_length<<2));
        ...
        if(tcpHdr->src()==80||tcpHdr->dst()==80){
            if(search(tcpPayload, tcpPayload+tcpLength, pattern, pattern+strlen(pattern))==tcpPayload+tcpLength) continue;
